...
def get_message(message, dynamodb=None):
    ...
    if not dynamodb:
        dynamodb = boto3.resource('dynamodb', endpoint_url=environ["DYNAMODB_ENDPOINT_OVERRIDE"])
...
async def decrypt(user_message, service_message):
    ...
    try: 
        # decrypted_user_mesage, decrypted_service_mesage = await asyncio.gather(
        #     crypto.unpack_message(wallet_handle, user_message.encode("utf-8")), 
        #     crypto.unpack_message(wallet_handle, service_message.encode("utf-8"))
        # )
        decrypted_user_mesage  = await crypto.unpack_message(wallet_handle, user_message.encode("utf-8"))
        decrypted_service_mesage = await crypto.unpack_message(wallet_handle, service_message.encode("utf-8"))
    except IndyError as e:
        logger.error(e)

    return json.loads(decrypted_user_mesage.decode("utf-8")), json.loads(decrypted_service_mesage.decode("utf-8"))
...
def validate_data(decrypted_user_mesage, decrypted_service_mesage):
    ...
    try:
        ...
        assert decrypted_user_mesage["timestamp"] == decrypted_service_mesage["timestamp"]

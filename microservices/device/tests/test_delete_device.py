...
def test_delete_device_ok(device, session):
    res = delete_device(device_id=device.id, connection=session)
    assert res["statusCode"] == 201


def test_delete_device_not_found(session):
    res = delete_device(device_id=0, connection=session)
